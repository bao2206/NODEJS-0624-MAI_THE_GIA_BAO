...
class MainService {
    ...
    applyDiscountToCart = async(idCart, discountCode) =>{
        const [isDiscountValid, discount, cart] = await Promise.all([
            DiscountService.checkCodeDiscount(discountCode),
            DiscountModel.findOne({code: discountCode.trim()}),
            this.findCartById(idCart),
        ])
        if (!isDiscountValid) {
            return { success: false, message: "Discount code is invalid or does not exist." };
          }
